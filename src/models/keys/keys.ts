import { projectPrefix } from '@/config/config.model'
 ...
 * 例如 localStorage.setItem('ONDUTY_FE_API', 'http://99.99.99.99:9999/')
 ...
 * 【优先级高于 KEY_API】
 * 这里的配置可以使得在不方便改代码进行调试时，可以在浏览器控制台输入相应的代码进行路径的切换
 * 例如 localStorage.setItem('ONDUTY_FE_SUB_API_/staff/', 'http://99.99.99.99:9999/')
 ...
 * 例如 localStorage.setItem('ONDUTY_FE_OPEN_STATIC', '1') // 取静态数据
 * localStorage.setItem('ONDUTY_FE_OPEN_STATIC', '0') // 不取静态数据
 */
...
// 例如：localStorage.setItem('ONDUTY_FE_STATIC_INCLUDE', '["campus/", "staff/"]')
